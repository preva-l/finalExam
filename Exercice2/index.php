<?php
// Second error, the file name require was wrong..
require_once 'connect.php';


$order = '';
//9th error, variable not set (not mandatory..)
$errors = "";
// Third error, ")" was missing in the if$GET
if(isset($_GET['order']) && isset($_GET['column'])){
	// 11th error order and column ..
	if($_GET['column'] == 'lastname'){$order = ' ORDER BY lastname';}
	// 13th error == at firstname
...
if(!empty($_POST)){
	...
	if(strlen($post['firstname'] < 3)){
		//9th error (suite) concatenation for the variable
	...
	if(empty($post['city'])){
		...
		// 12th error, missing 's' to $errors
	if(count($errors) > 0){
			...
			// 5th error "r" missing to the second firstname..
		...
		if($insertUser->execute()){
			...
			$errors = 'Erreur SQL';
		}//7th error "}" missing.. (ifcount)
}//8th error "}" missing.. (ifpost)





// 10th error, $queryuser "blocked" in the if count
...
<body>
	...
	<div class="container">
		...
		<div class="row">
			...
			// 14th error, missing 's' to error, !empty
			if(!empty($errors)){
				echo '<div class="col-md-6 col-md-offset-3">';
			...
			<div class="col-md-5">
<!-- Form action  -->
				<form method="post"  action="#" class="form-horizontal well well-sm">
					<fieldset>
